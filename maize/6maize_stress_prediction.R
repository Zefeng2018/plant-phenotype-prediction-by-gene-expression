...
#####################################
library(caret)
HVG_tpm_sub <- subset(HVG_tpm,HVG_tpm$class %in% c("cold","drought","control","heat"))
...
######################################################################
## 1.  confusion matrix
svm_cm <- confusionMatrix(svm_pred,test_data$class)  # AC:0.95
xgb_cm <- confusionMatrix(as.factor(predict(xgb, dtest)),as.factor(label_test)) # 0.96
rf_cm <- confusionMatrix(rf_pred,test_data$class) # 0.97
nb_cm <- confusionMatrix(nb_pred,test_data$class) # 0.87
...
dnn_cm <- confusionMatrix(dnn_pre,dnn_true) # 0.96
#colnames(dnn_cm$table)<-rownames(dnn_cm$table)<-c("cold","control", "drought","heat")
colnames(xgb_cm$table)<-rownames(xgb_cm$table)<-c("cold","control", "drought","heat")
...
p<-ggplot(subdata_long,aes(x=class,y=log(Expression+1,2),fill=Gene))+
  ...
  xlab("Stress type")+
  theme(legend.position = c(0.15,0.9),text = element_text(size = 8),
        axis.title =element_text(size=10))
ggsave(p,file="results_pictures/Stree_xgb_feature_gene_expression.pdf", width = 105, height = 105, units = "mm")



## random forest
a<-as.data.frame(varImpPlot(rf_model))
b<-a%>%arrange(desc(MeanDecreaseAccuracy))
library(ggsci)
subdata <- train_data[,colnames(train_data)%in%c(rownames(b[1:5,]),"class")]
subdata_long <- gather(data = subdata,key = "Gene",value = "Expression",-class)
