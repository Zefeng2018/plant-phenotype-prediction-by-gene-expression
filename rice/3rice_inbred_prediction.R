...
HVG_tpm_sub <- subset(HVG_tpm_sub,HVG_tpm_sub$class%in%c("Nipponbare","Horkuch mother(sensitive)","IRAT109","IR29 mother(tolerant)","Horkuch mother(tolerant)"))
...
xgb <- xgboost(data = dtrain,max_depth=6, eta=0.5, objective='multi:softmax',num_class=5, nround=25,verbose = 1)
...
y_train = HVG_tpm_sub_keras[train_sub,] %>% pull(class) %>% to_categorical(5)
x_test  = HVG_tpm_sub_keras[-train_sub,-ncol(HVG_tpm_sub_keras)] %>% as.matrix
y_test  = HVG_tpm_sub_keras[-train_sub,]  %>% pull(class) %>% to_categorical(5)
...
######################################################################
## 1.  confusion matrix
svm_cm <- confusionMatrix(svm_pred,test_data$class)  # AC:0.8925
xgb_cm <- confusionMatrix(as.factor(predict(xgb, dtest)),as.factor(label_test)) # 0.9159
rf_cm <- confusionMatrix(rf_pred,test_data$class) # 0.9206
nb_cm <- confusionMatrix(nb_pred,test_data$class) # 0.7009
...
dnn_cm <- confusionMatrix(dnn_pre,dnn_true) # 0.8925
...
library(ggsci)
subdata <- train_data[,colnames(train_data)%in%c(rownames(import)[1:5],"class")]
